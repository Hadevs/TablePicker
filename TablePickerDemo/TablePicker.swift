...
class TablePicker: NSObject {
    static func show(withData data: [String], isMultiple: Bool?, title: String, andCompletion completion: @escaping ([String]) -> Void) {
        ...
        guard let window = UIApplication.shared.keyWindow, let rootVC = window.rootViewController else {
            NSLog("Error with presenting TablePickerController().")
            return
        }
